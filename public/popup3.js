...
// 검사 시작 버튼을 눌러서 모달이 나타나고 모달의 동의 버튼을 누르면 scrollAndExtract()가 실행되면서 검사 시작버튼은 검사 중단 버튼으로 토글
// 검사 중단 버튼을 누르면 stopScript() 실행이 되면서 검사 시작 버튼으로 토글
// 검사 중단 버튼을 누른 후 다시 검사 시작 버튼을 눌렀을 때 scrollAndExtract()가 실행 되어야 함
// scrollAndExtract()가 끝나면 검사 중단 버튼이 검사 시작 버튼으로 토글
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  button.addEventListener('click', () => {
    ...
    if (isScanning) {
      ...
      isScanning = !isScanning; // 토글
      button.textContent = isScanning ? '검사 시작' : '검사 중단';  // 버튼 변경
        ...
        chrome.scripting.executeScript({
          ...
          func: () => {
            if (typeof scrollAndExtract === 'function') {
              ...
              button.textContent = isScanning ? '검사 시작' : '검사 중단';
              // content.js 실행
              scrollAndExtract();
            } else {
